...
test("release checkpoint keeps only non-sensitive rollback metadata", () => {
  const checkpoint = buildReleaseCheckpoint({
    ...
    appsScriptVersion: production.snapshot,
  ...
  assert.equal(checkpoint.appsScript.numberedVersion, production.snapshot);
...
test("release checkpoint rejects runtime hash and numbered-version drift", () => {
  ...
  assert.throws(() => buildReleaseCheckpoint({ ...input, appsScriptVersion: "v50" }),
    /production pointer/);
});
