...
import {verifyScheduledReleaseInventory} from '../apps-script/scripts/release-v54.mjs';
  ...
  return {
    ...
    newTrigger: handler => {
      ...
      return {
        ...
        create() {
          const trigger = clockTrigger(handler, 'v54-' + nextId++, {...schedule});
...
test('v54 runtime identity fingerprints the stabilization candidate', () => {
  const f = loadBundle('v54');
  const identity = JSON.parse(JSON.stringify(f.context.getDmsRuntimeIdentity_()));
  assert.equal(identity.release, 'calendar-ingestion-stabilization');
  assert.deepEqual({
  ...
  }, runtimeSourceHashes('apps-script/candidates/v54'));
});

test('v54 does not mark the 08:00 morning job stale at 04:10 Moscow', () => {
  const {f} = configuredV54();
...
test('v54 daily job becomes delayed after grace and stale only after the stale threshold', () => {
  const {f} = configuredV54();
...
test('v54 daily success makes the current window healthy with execution metadata', () => {
  const {f, manifest} = configuredV54();
...
test('v54 records a redacted error class and exposes last_run_failed', () => {
  const {f, manifest} = configuredV54(new Date(Date.now() - 7200000).toISOString());
...
test('v54 reports trigger missing separately from freshness', () => {
  const {f, ScriptApp} = configuredV54();
...
test('v54 defers only the backup age warning while its execution window is open', () => {
  const {f} = configuredV54();
...
test('v54 administrator health text is compact and contains no trigger identifiers', () => {
  const {f} = configuredV54();
  ...
  assert.equal(text.includes('v54-'), false);
});

test('v54 release inventory requires the existing owner-bound trigger set', () => {
  const now = Date.now();
...
test('v54 deduplicates one state but surfaces a new scheduled error class', () => {
  const {f} = configuredV54();
  const authorization = [{
