...
// One bounded durable record, written only by the ingestion owner under ScriptLock.
// No pending-observation writes or repairs are performed by the watchdog.
const DMS_CALENDAR_GENERATION_KEY = 'DMS_CALENDAR_SYNC_GENERATION_V1';
const DMS_CALENDAR_INGESTION_CLASSES = [
  'pendingMissingCalendar', 'calendarQueueTimeMismatch',
  'queuePointsToNonTraining', 'calendarTrainingMissingQueue'
];

function readDmsCalendarSyncGeneration_() {
  const raw = PropertiesService.getScriptProperties().getProperty(DMS_CALENDAR_GENERATION_KEY);
  if (!raw) return {syncGeneration: 0};
  try {
    const value = JSON.parse(raw);
    if (value.version !== 1 || !Number.isSafeInteger(value.syncGeneration) ||
        value.syncGeneration < 0 || !Array.isArray(value.driftKeys) || value.driftKeys.length > 64) {
      return {syncGeneration: 0, status: 'failed'};
    }
    return value;
  } catch (error) { return {syncGeneration: 0, status: 'failed'}; }
}

function saveDmsCalendarSyncGeneration_(value) {
  PropertiesService.getScriptProperties().setProperty(DMS_CALENDAR_GENERATION_KEY, JSON.stringify(value));
}

function beginDmsCalendarSyncGeneration_() {
  const previous = readDmsCalendarSyncGeneration_();
  const value = {
    version: 1, syncGeneration: previous.syncGeneration,
    lastSyncStarted: new Date().toISOString(),
    lastSyncCompleted: previous.lastSyncCompleted || '',
    status: 'running', lastError: '', durationMs: null,
    driftKeys: previous.driftKeys || [], postSync: previous.postSync || null
  };
  saveDmsCalendarSyncGeneration_(value);
  return value;
}

function getDmsCalendarIngestionEvidence_(report) {
  const keys = [];
  let immediateCount = 0;
  Object.keys(report.issues || {}).sort().forEach(function(name) {
    const entries = report.issues[name] || [];
    if (DMS_CALENDAR_INGESTION_CLASSES.indexOf(name) === -1) {
      immediateCount += entries.length;
      return;
    }
    entries.forEach(function(issue) {
      const bytes = Utilities.computeDigest(Utilities.DigestAlgorithm.SHA_256,
        JSON.stringify([name, issue]), Utilities.Charset.UTF_8);
      const hash = Utilities.base64EncodeWebSafe(bytes);
      if (keys.indexOf(hash) === -1) keys.push(hash);
    });
  });
  return {keys: keys, immediateCount: immediateCount};
}

function completeDmsCalendarSyncGeneration_(value, before, report) {
  const after = getDmsCalendarIngestionEvidence_(report);
  const drift = after.keys.filter(function(key) { return before.keys.indexOf(key) !== -1; });
  value.syncGeneration++;
  value.lastSyncCompleted = new Date().toISOString();
  value.durationMs = Date.parse(value.lastSyncCompleted) - Date.parse(value.lastSyncStarted);
  value.status = 'succeeded';
  value.driftKeys = drift.slice(0, 64);
  value.postSync = {
    checkedAt: value.lastSyncCompleted, issueCount: report.issueCount,
    driftCount: drift.length, pendingCount: after.keys.length - drift.length,
    immediateCount: after.immediateCount, overflow: drift.length > 64
  };
  saveDmsCalendarSyncGeneration_(value);
}

function failDmsCalendarSyncGeneration_(value) {
  value.status = 'failed';
  value.lastError = 'sync_execution_failed';
  value.durationMs = new Date().getTime() - Date.parse(value.lastSyncStarted);
  saveDmsCalendarSyncGeneration_(value);
}

function classifyDmsCalendarIngestion_(value, report, job) {
  const evidence = getDmsCalendarIngestionEvidence_(report);
  const post = value.postSync;
  const matchingDrift = evidence.keys.filter(function(key) {
    return (value.driftKeys || []).indexOf(key) !== -1;
  }).length;
  let state = 'healthy';
  let ok = true;
  if (!job || !job.configOk || job.state === 'trigger_missing') {
    state = 'sync_trigger_missing'; ok = false;
  } else if (job.state === 'last_run_failed' || value.status === 'failed') {
    state = 'sync_failed'; ok = false;
  } else if (!job.fresh) {
    state = 'sync_delayed'; ok = false;
  } else if (evidence.immediateCount) {
    state = 'accounting_drift'; ok = false;
  } else if (value.status !== 'running' && post && (matchingDrift || post.overflow)) {
    state = 'drift_after_successful_sync'; ok = false;
  } else if (value.status === 'running' || evidence.keys.length || !post) {
    state = job.state === 'not_due_yet' && !evidence.keys.length ? 'not_due_yet' : 'awaiting_sync';
  }
  return {
    ok: ok, state: state, syncGeneration: value.syncGeneration,
    lastSyncStarted: value.lastSyncStarted || '', lastSyncCompleted: value.lastSyncCompleted || '',
    durationMs: value.durationMs == null ? null : value.durationMs,
    lastError: value.lastError || '', nextExpectedWindow: job && job.nextExpectedWindow || null,
    postSync: post || null, pendingCount: Math.max(0, evidence.keys.length - matchingDrift),
    driftCount: state === 'drift_after_successful_sync' ? matchingDrift : 0,
    summary: state + (state === 'drift_after_successful_sync' ? '; issues=' + matchingDrift : '')
  };
}
