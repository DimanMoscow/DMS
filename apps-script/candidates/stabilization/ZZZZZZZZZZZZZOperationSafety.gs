...
function withTelegramDocumentLock_(callback, metrics) {
  const lease = getDmsMutationLock_();
  const startedAt = Date.now();
  if (!lease.tryLock(10000)) {
...
function getDmsConfirmationCalendarTargets_(payload) {
  ...
  return targets.map(function(target) {
    let event;
    try {
      event = measureActiveDmsOperationPhase_('calendarMs', function() {
        return Calendar.Events.get(target.calendarId, target.eventId);
      });
    }
    catch (error) {
...
function dmsCalendarConditionalWrite_(method, calendarId, eventId, resource) {
  ...
  const response = measureActiveDmsOperationPhase_('calendarMs', function() {
    return UrlFetchApp.fetch('https://www.googleapis.com/calendar/v3/calendars/' +
...
function dmsCalendarPatch_(resource, calendarId, eventId, options) {
  if (!DMS_CONFIRMED_EXECUTION) {
    return measureActiveDmsOperationPhase_('calendarMs', function() {
...
function dmsCalendarRemove_(calendarId, eventId, options) {
  if (!DMS_CONFIRMED_EXECUTION) {
    return measureActiveDmsOperationPhase_('calendarMs', function() {
      return Calendar.Events.remove(calendarId, eventId, options);
    });
  }
...
function processTelegramSecureCallback_(query, parsed, nowMs) {
  ...
  finishDmsOperationMetricsSafely_(metrics,
    result.status === 'manual_review' || result.code === 'day_partial' ? 'partial' :
