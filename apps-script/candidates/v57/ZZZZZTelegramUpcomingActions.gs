...
function handleTelegramOpsCallback_(query, data, userId, chatId, messageId) {
  ...
  try {
    ...
    } else if (data.indexOf('ops:trainings:') === 0) {
      const parts = data.split(':');
      sendTelegramClientTrainingHistory_(chatId, parts[2], messageId, Number(parts[3]) || 0);
...
function sendTelegramClientCard_(chatId, clientId, messageId) {
  ...
  keyboard.push([{text: '📋 История тренировок', callback_data: 'ops:trainings:' + card.id + ':0'}]);
  keyboard.push([
