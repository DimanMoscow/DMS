...
function isDmsBackupAgeAwaitingExpectedWindow_(backup, backupJob) {
  return !!backupJob && !backup.ok && Array.isArray(backup.issues) &&
    backup.issues.length > 0 && backup.issues.every(function(issue) {
      return issue === 'Последняя копия старше 48 часов';
    }) && (backupJob.state === 'not_due_yet' ||
      backupJob.state === 'within_expected_window');
}

function runDmsReadOnlySelfTests(options) {
  ...
  try {
    ...
    const health = profileDmsHealthPhase_('system', function() { return getDmsSystemHealth(); });
    ...
    const debtFormula = profileDmsHealthPhase_('financial', function() { return getDmsDebtFormulaHealth_(); });
    const semanticReconciliation = profileDmsHealthPhase_('semantic', function() { return buildDmsSemanticReconciliationHealth_(debtFormula); });
    report.semanticReconciliation = semanticReconciliation;
    const queueSourceValidation = profileDmsHealthPhase_('queue_validation', function() { return getDmsQueueSourceValidationHealth_(); });
    addDmsSelfTestCheck_(
      report,
    ...
    const scheduled = profileDmsHealthPhase_('schedules', function() { return getDmsScheduledAutomationHealth_({requireFreshness: true}); });
    report.scheduledAutomation = scheduled;
    addDmsSelfTestCheck_(
      report,
      'scheduled-trigger-config',
      scheduled.configOk,
      scheduled.configOk ? 'handlers, owner, clock source, schedule and timezone verified' :
        'scheduled trigger configuration is not verified'
    );
    addDmsSelfTestCheck_(
      report,
      'scheduled-notification-settings',
      scheduled.settingsOk,
      scheduled.settingsOk ? 'morning/evening enabled' : 'morning/evening setting disabled'
    );
    const scheduledProblems = scheduled.handlers.filter(function(item) {
      return !item.fresh;
    }).map(function(item) {
      return item.handler + '=' + item.state +
        (item.lastErrorClass ? ':' + item.lastErrorClass : '');
    });
    addDmsSelfTestCheck_(
      report,
      'scheduled-trigger-freshness',
      scheduled.freshnessOk,
      scheduledProblems.length ? scheduledProblems.join(', ') :
        'all scheduled handlers are healthy or still inside their expected windows'
    );

    const clients = getRequiredSheet_(ss, DMS_QUEUE_PROCESSING.CLIENTS);
    const blocks = getRequiredSheet_(ss, DMS_QUEUE_PROCESSING.BLOCKS);
    const log = getRequiredSheet_(ss, DMS_QUEUE_PROCESSING.LOG);
    const context = profileDmsHealthPhase_('accounting_context', function() { return buildQueueProcessingContext_(clients, blocks, log); });
    ...
    const duplicateKeys = profileDmsHealthPhase_('duplicates', function() { return findDmsDuplicateAccountingKeys_(log); });
    addDmsSelfTestCheck_(
      report,
    ...
    // Snapshot the generation BEFORE reading Calendar/Queue. A concurrent sync
    // completing after this read cannot promote an older observation to drift.
    const syncEvidence = readDmsCalendarSyncGeneration_();
    const reconciliation = profileDmsHealthPhase_('calendar_reconciliation', function() { return runDmsCalendarQueueReconciliation(); });
    ...
    addDmsSelfTestCheck_(
      ...
      options && options.watchdog ? calendarHealth.ok : reconciliation.ok && calendarHealth.ok && !!syncEvidence.postSync && syncEvidence.status === 'succeeded',
      calendarHealth.summary
    );

    const backup = profileDmsHealthPhase_('backup', function() { return validateLatestDmsBackup(); });
    const backupJob = scheduled.handlers.find(function(item) {
      return item.handler === 'createDmsAutomaticBackup';
    });
    ...
    addDmsSelfTestCheck_(
      ...
      backup.ok || backupAwaitingWindow,
      backupAwaitingWindow
        ? 'Возраст будет оценён после текущего допустимого окна backup'
        : backup.summary
    );
    ...
    const durable = profileDmsHealthPhase_('durable', function() { return getDmsDurableOperationHealth_(); });
    ...
    const measurementHealth = profileDmsHealthPhase_('measurements', function() { return getDmsMeasurementCorruptionHealth_(); });
...
function runDmsCalendarQueueReconciliation(metrics, observation) {
  const ss = SpreadsheetApp.getActive();
  const queue = getRequiredSheet_(ss, DMS_QUEUE_PROCESSING.QUEUE);
  const log = getRequiredSheet_(ss, DMS_QUEUE_PROCESSING.LOG);
  const settings = getRequiredSheet_(ss, DMS_SYNC.SETTINGS);
  ...
  const readSheets = function() {
    ...
    queueRows = readDmsReconciliationRows_(
      queue,
      DMS_QUEUE_PROCESSING.QUEUE_FIRST_ROW,
      DMS_QUEUE_PROCESSING.QUEUE_COLUMNS
    );
    logRows = readDmsReconciliationRows_(
      log,
      DMS_QUEUE_PROCESSING.LOG_FIRST_ROW,
      DMS_QUEUE_PROCESSING.LOG_COLUMNS
    );
    if (metrics) addDmsOperationCount_(
      metrics,
      'rowsRead',
      queueRows.length + logRows.length
    );
  };
  ...
  if (observation) {
    observation.calendarId = config.calendarId;
    observation.start = window.start;
    observation.end = window.end;
  ...
  console.log(JSON.stringify({ok: report.ok, checkedAt: report.checkedAt, issueCount: report.issueCount, safeRepairCount: report.safeRepairCount}));
  return report;
}
  ...
  (queueRows || []).forEach(function(item) {
    ...
    if (
      ...
      if (!event || event.status === 'cancelled') {
        issues.pendingMissingCalendar.push({queueId: queueId, row: item.row, revision: event ? String(event.etag || event.updated || '') : 'absent'});
      } else if (!isTrainingEventTitle_(event.summary || '')) {
        issues.queuePointsToNonTraining.push({queueId: queueId, row: item.row, revision: String(event.etag || event.updated || ''), title: String(event.summary || '')});
      } else {
        ...
        if (
          ...
          issues.calendarQueueTimeMismatch.push({queueId: queueId, row: item.row, revision: String(event.etag || event.updated || ''), start: times.start, end: times.end});
        }
      }
    }
  });
  ...
  (calendarEvents || []).forEach(function(event) {
    ...
    if (
      ...
      issues.calendarTrainingMissingQueue.push({
        eventId: eventId,
        revision: String(event.etag || event.updated || ''),
        start: times.start,
...
function runDmsBackupSetupV31() {
  ...
  if (!validation.ok) {
    throw new Error('Последняя копия не прошла проверку: ' + validation.summary);
  }
  const result = {
    trigger: trigger,
    backupCreated: false,
...
function getDmsWatchdogAlertSignature_(failedChecks) {
  return JSON.stringify((failedChecks || []).map(function(check) {
    return [String(check.name || ''), String(check.details || '')];
  }));
}
  ...
  try {
    ...
    const report = withDmsOperationMetrics_(metrics, function() {
      ...
      return measureDmsOperationPhase_(metrics, 'healthCheckMs', function() {
        return runDmsReadOnlySelfTests({watchdog: true});
      });
    });
    ...
    if (report.ok) {
      ...
      recordDmsScheduledAutomationSuccess_(
        'runDmsWatchdog', event, 'healthy', execution);
    ...
    const signature = getDmsWatchdogAlertSignature_(failed);
    ...
    if (signature === previousSignature && previousSentAt && now - previousSentAt < repeatAfterMs) {
      recordDmsScheduledAutomationSuccess_(
        'runDmsWatchdog', event, 'deduplicated', execution);
      finishDmsOperationMetricsSafely_(metrics, 'partial');
    ...
    recordDmsScheduledAutomationSuccess_(
      'runDmsWatchdog', event, 'alerted', execution);
    finishDmsOperationMetricsSafely_(metrics, 'partial');
    return {ok: false, notified: true, summary: report.summary};
  } catch (error) {
    finishDmsOperationMetricsSafely_(metrics, 'failed', error);
    finishDmsScheduledAutomationFailure_('runDmsWatchdog', event, error, execution);
    throw error;
  }
}
...
function runDmsMonitoringSetupV32() {
  ...
  const check = getDmsScheduledAutomationHealth();
...
// Only fixed phase labels and durations leave this profiler; no row data.
function profileDmsHealthPhase_(phase, work) {
  const started = Date.now();
  try { return work(); }
  finally { console.log(JSON.stringify({event: 'dms_health_phase', phase: phase, durationMs: Date.now() - started})); }
}
