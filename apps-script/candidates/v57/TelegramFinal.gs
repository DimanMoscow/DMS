...
function buildTelegramSystemHealthText_(report, health) {
  ...
  const labels = {
    sendTelegramMorningDigest: 'Утро 08:00',
    sendTelegramDailyQueue: 'Вечер 22:00',
    createDmsAutomaticBackup: 'Backup 03:00',
    syncCalendarToQueue: 'Calendar каждый час'
  };
  scheduled.handlers.filter(function(item) {
    return labels[item.handler];
  }).forEach(function(item) {
    const stateLabels = {
      trigger_missing: 'отсутствует',
      trigger_misconfigured: 'неверная конфигурация',
      not_due_yet: 'ещё не пора',
      within_expected_window: 'в допустимом окне',
      delayed: 'задерживается',
      last_run_failed: 'последний запуск завершился ошибкой',
      last_run_succeeded: 'выполнен',
      stale: 'просрочен'
    };
    const success = item.lastSuccessAt
      ? Utilities.formatDate(new Date(item.lastSuccessAt),
        DMS_SCHEDULED_AUTOMATION.TIME_ZONE, 'dd.MM HH:mm')
      : 'нет';
    const duration = item.durationMs === null
      ? 'нет'
      : String(Math.round(item.durationMs / 100) / 10) + ' с';
    const error = item.lastErrorClass ? '; error ' + item.lastErrorClass : '';
    lines.push(labels[item.handler] + ': ' +
      (item.fresh ? '✅ ' : '⚠️ ') + (stateLabels[item.state] || item.state) +
      '; успех ' + success + '; длит. ' + duration + error);
  });
...
function sendTelegramMorningDigest(event) {
  ...
  try {
    ...
    const result = withDmsOperationMetrics_(metrics, function() {
      ...
      if (!settings.morning) {
        recordDmsScheduledAutomationSuccess_(
          'sendTelegramMorningDigest', event, 'disabled', execution);
        return 'Утренняя сводка отключена.';
      }
      validateTelegramConfiguration_();
      measureDmsOperationPhase_(metrics, 'calendarSyncMs', function() {
        return syncCalendarToQueue();
      });
      const chatId = getTelegramProperty_(DMS_TELEGRAM.PROP_CHAT_ID);
      let dashboard;
      let attention;
      measureDmsOperationPhase_(metrics, 'sheetsReadMs', function() {
        dashboard = buildTelegramQueueDashboard_(new Date());
        attention = buildTelegramAttentionReport_();
      });
      telegramSendMessage_(chatId,
        '<b>Доброе утро</b>\n\n' + dashboard.text + '\n\n' + attention.text,
        dashboard.replyMarkup);
      recordDmsScheduledAutomationSuccess_(
        'sendTelegramMorningDigest', event, 'sent', execution);
    ...
    finishDmsScheduledAutomationFailure_(
      'sendTelegramMorningDigest', event, error, execution);
    throw error;
  }
}
...
function sendTelegramDailyQueue(event) {
  ...
  try {
    ...
    const result = withDmsOperationMetrics_(metrics, function() {
      ...
      if (!settings.evening) {
        recordDmsScheduledAutomationSuccess_(
          'sendTelegramDailyQueue', event, 'disabled', execution);
        return 'Вечерняя проверка отключена.';
      }
      validateTelegramConfiguration_();
      measureDmsOperationPhase_(metrics, 'calendarSyncMs', function() {
        return syncCalendarToQueue();
      });
      const dashboard = measureDmsOperationPhase_(
        metrics,
        'sheetsReadMs',
        function() { return buildTelegramQueueDashboard_(new Date()); }
      );
      if (!dashboard.items.length) {
        recordDmsScheduledAutomationSuccess_(
          'sendTelegramDailyQueue', event, 'empty', execution);
        return 'Необработанных тренировок нет.';
      }
      telegramSendMessage_(getTelegramProperty_(DMS_TELEGRAM.PROP_CHAT_ID),
        '<b>Остались необработанные тренировки</b>\n\n' + dashboard.text,
        dashboard.replyMarkup);
      recordDmsScheduledAutomationSuccess_(
        'sendTelegramDailyQueue', event, 'sent', execution);
    ...
    finishDmsOperationMetricsSafely_(metrics, 'failed', error);
    finishDmsScheduledAutomationFailure_(
      'sendTelegramDailyQueue', event, error, execution);
    throw error;
  }
}
...
function createTelegramDataBackup(metrics) {
  const lock = getDmsMutationLock_();
  const lockStartedAt = Date.now();
  if (!lock.tryLock(10000)) {
    if (metrics) addDmsOperationDuration_(metrics, 'lockWaitMs', Date.now() - lockStartedAt);
  ...
  try {
    ...
    const rows = metrics
      ? measureDmsOperationPhase_(metrics, 'sheetsReadMs', function() {
        return buildDmsBackupRows_(ss, backupId, createdAt, metrics);
      })
      : buildDmsBackupRows_(ss, backupId, createdAt);
    const firstRow = Math.max(target.getLastRow() + 1, 2);

    const writeBackup = function() {
      target.getRange(firstRow, 1, rows.length, 5).setValues(rows);
      SpreadsheetApp.flush();
      if (metrics) addDmsOperationCount_(metrics, 'rowsWritten', rows.length);
    };
    if (metrics) measureDmsOperationPhase_(metrics, 'sheetsWriteMs', writeBackup);
    else writeBackup();

    const validation = metrics
      ? measureDmsOperationPhase_(metrics, 'sheetsReadMs', function() {
        return validateDmsBackupById_(target, backupId, createdAt, metrics);
      })
      : validateDmsBackupById_(target, backupId, createdAt);
    if (!validation.ok) {
    ...
    if (metrics) measureDmsOperationPhase_(metrics, 'sheetsWriteMs', audit);
    else audit();
    return backupId;
  } finally {
    lock.releaseLock();
...
function buildDmsBackupRows_(ss, backupId, createdAt, metrics) {
  const rows = [];
  ...
  getDmsBackupSheetNames_().forEach(function(name) {
    ...
    if (metrics) addDmsOperationCount_(metrics, 'rowsRead', range.getNumRows());
    const json = JSON.stringify(
      serializeTelegramUndoValues_(range.getValues())
    );
...
function validateDmsBackupById_(target, backupId, now, metrics) {
  const lastRow = target.getLastRow();
  if (metrics) addDmsOperationCount_(metrics, 'rowsRead', Math.max(0, lastRow - 1));
...
function createDmsAutomaticBackup(event) {
  ...
  try {
    ...
    recordDmsScheduledAutomationSuccess_(
      'createDmsAutomaticBackup', event, 'completed', execution);
    finishDmsOperationMetricsSafely_(metrics, 'success');
    return backupId;
  } catch (error) {
    finishDmsOperationMetricsSafely_(metrics, 'failed', error);
    finishDmsScheduledAutomationFailure_(
      'createDmsAutomaticBackup', event, error, execution);
    throw error;
  }
}
