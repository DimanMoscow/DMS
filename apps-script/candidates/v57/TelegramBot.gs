...
const DMS_RUNTIME_IDENTITY = {
  ...
  ROUTER_SHA256: "161d134be312bf1e35206e31f290c670f930b16daaa9c3f50559b6e5d7dd4866",
  CLIENT_PORTAL_SHA256: '763e56aebc3bd07db8bae8e70e33e40ea3ab29856f7fb9ed408c482e979e4b98',
  TELEGRAM_CONFIRMATIONS_SHA256: "b2e901820acae7e6453d57d562d244caeb916abb1a190583c95e0318a1460ba5"
};
...
function buildTelegramQueueDashboard_(date) {
  ...
  items.forEach(function(item, index) {
    ...
    const block = item.blockId ? ' · ' + item.blockId :
      item.matching === DMS_UNKNOWN_CLIENT_STATUS ? ' · требуется регистрация' : ' · без блока';
...
function telegramApi_(method, payload) {
  ...
  const response = measureActiveDmsOperationPhase_('telegramMs', function() {
...
function telegramSendMessageV49_(chatId, text, replyMarkup) {
  const payload = {
...
function telegramEditMessageV49_(chatId, messageId, text, replyMarkup) {
  const payload = {
    chat_id: String(chatId),
...
function startTelegramMove_(userId, chatId, dashboardMessageId, item) {
  const state = {
    secureFlowId: makeTelegramSecureFlowId_(),
...
function confirmTelegramSchedule_(userId, chatId, messageId, allowConflict) {
  ...
  try {
    ...
    if (conflicts.length && !allowConflict) {
      if (DMS_TELEGRAM_SECURE_DELIVERY) {
        throw new Error('Расписание изменилось. Открой создание тренировки заново.');
      }
      showTelegramScheduleConfirmation_(chatId, messageId, state);
      return;
    }
...
function sendTelegramAttentionCenter_(chatId) {
  ...
  const onboarding = getDmsTelegramOnboardingNextAction_();
  if (onboarding.replyMarkup) keyboard.push.apply(keyboard, onboarding.replyMarkup.inline_keyboard);
  telegramSendMessage_(chatId, report.text + onboarding.text, keyboard.length ? {inline_keyboard: keyboard} : null);
}
    ...
    clientsSheet.getRange(
      ...
      if (row[3]) {
        ...
        if (block && block.status === 'Приостановлен') {
          ...
          affected[client.id] = client;
        } else if (block && block.status === 'Запланирован' &&
