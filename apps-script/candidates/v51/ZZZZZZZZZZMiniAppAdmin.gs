...
function resolveDmsCalendarOnboarding_(payload, actorId) {
  ...
  try {
    ...
    if (mode === 'link' && item.values[11] === 'Распознано' &&
        String(item.values[8] || '') === String(request.clientId || '').trim()) {
      return {changed: false, queueId: item.queueId, mode: mode};
    }
    if (mode === 'new' && item.values[11] === 'Распознано' && item.values[8]) {
      const requestedName = String(request.name || '').replace(/\s+/g, ' ').trim();
      const resolvedName = String(item.values[9] || '').replace(/\s+/g, ' ').trim();
      if (requestedName && requestedName === resolvedName) {
        return {
          changed: false,
          queueId: item.queueId,
          mode: mode,
          clientId: String(item.values[8])
        };
      }
    }
    ...
    const preview = buildDmsCalendarOnboardingPreview_(ss, item, request);
