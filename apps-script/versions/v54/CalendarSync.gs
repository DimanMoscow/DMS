...
function syncCalendarToQueue(event) {
  ...
  try {
    ...
    try {
      ...
      try {
      const before = getDmsCalendarIngestionEvidence_(runDmsCalendarQueueReconciliation());
      const plan = buildCalendarQueueSyncPlan_();
      if (plan.errors) throw new Error('Calendar sync plan incomplete.');
      const result = applyCalendarQueueSyncPlan_(plan);
      SpreadsheetApp.flush();
      const after = runDmsCalendarQueueReconciliation();
