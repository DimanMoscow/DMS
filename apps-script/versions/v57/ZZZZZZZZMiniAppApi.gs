...
function handleDmsMiniAppRequest_(body) {
  try {
    ...
    else if (action === 'preview_calendar_onboarding') {
      data = previewDmsCalendarOnboarding_(payload);
    }
    ...
    // Never emit request payloads, signed initData, Telegram identities, invite
    // tokens, Calendar titles, or client names into the execution log.
    console.error(JSON.stringify({
      event: 'mini_app_api_failure',
      action: normalizeDmsMiniAppLogAction_(body && body.action),
      code: failure.code,
...
function normalizeDmsMiniAppLogAction_(value) {
  const action = String(value || 'bootstrap');
  return /^[a-z0-9_]{1,64}$/.test(action) ? action : 'invalid_action';
}
...
function getDmsMiniAppBootstrap_() {
  ...
  const serializeQueueItem = function(item) {
    return {
  ...
  const waiting = queue.items.map(serializeQueueItem);
  const registrationQueue = queue.registrations.map(serializeQueueItem);
  ...
  return {
    ...
    summary: {
      ...
      queueRegistrations: queue.queueRegistrations,
...
function getDmsMiniAppQueueSnapshot_(date, ss, timeZone) {
  ...
  queue.getRange(
    ...
    if (status === DMS_UNKNOWN_CLIENT_STATUS) result.queueRegistrations++;
    if (!row[0] || !(row[1] instanceof Date)) return;
    const inDay = makeDateKey_(row[1], timeZone) === dateKey;
    const needsRegistration = status !== 'Обработано' && (status === DMS_UNKNOWN_CLIENT_STATUS || String(row[11] || '') === DMS_UNKNOWN_CLIENT_STATUS);
    if (!inDay && !needsRegistration) return;
    const item = {
      queueId: String(row[0]),
      dateLabel: Utilities.formatDate(row[1], timeZone, 'dd.MM.yyyy'),
      ...
      status: status,
      calendarTitle: String(row[7] || ''),
      needsRegistration: status === DMS_UNKNOWN_CLIENT_STATUS ||
        String(row[11] || '') === DMS_UNKNOWN_CLIENT_STATUS,
      processed: status === 'Обработано'
    };
    if (item.needsRegistration && !item.processed) result.registrations.push(item);
    if (makeDateKey_(row[1], timeZone) === dateKey) result.items.push(item);
  });
...
function getDmsMiniAppClient_(clientId) {
  ...
  return {
    ...
    trainingHistory: getDmsClientTrainingHistory_(id),
