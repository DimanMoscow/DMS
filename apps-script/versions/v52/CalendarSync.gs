...
function syncCalendarToQueue(event) {
  assertDmsScheduledInvocation_('syncCalendarToQueue', event);
  ...
  try {
    const result = applyCalendarQueueSyncPlan_(buildCalendarQueueSyncPlan_());
    recordDmsScheduledAutomationSuccess_('syncCalendarToQueue', event, 'completed');
    return result;
  } finally {
    lock.releaseLock();
  }
...
function buildCalendarQueueSyncPlan_() {
