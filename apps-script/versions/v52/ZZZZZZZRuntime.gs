...
function runDmsReadOnlySelfTests() {
  ...
  try {
    ...
    const staleScheduledHandlers = scheduled.handlers.filter(function(item) {
      return !item.fresh;
    ...
    addDmsSelfTestCheck_(
      ...
      staleScheduledHandlers.length ? 'stale: ' + staleScheduledHandlers.join(', ') :
        'all scheduled handlers completed within their expected windows'
    );
...
function runDmsWatchdog(event) {
  ...
  assertDmsScheduledInvocation_('runDmsWatchdog', event);
  ...
  if (report.ok) {
    ...
    recordDmsScheduledAutomationSuccess_('runDmsWatchdog', event, 'healthy');
  ...
  if (signature === previousSignature && previousSentAt && now - previousSentAt < repeatAfterMs) {
    recordDmsScheduledAutomationSuccess_('runDmsWatchdog', event, 'deduplicated');
  ...
  recordDmsScheduledAutomationSuccess_('runDmsWatchdog', event, 'alerted');
