...
function sendTelegramMorningDigest(event) {
  assertDmsP1ReleaseReady_();
  assertDmsScheduledInvocation_('sendTelegramMorningDigest', event);
  const settings = getTelegramFinalSettings_();
  if (!settings.morning) {
    recordDmsScheduledAutomationSuccess_('sendTelegramMorningDigest', event, 'disabled');
  ...
  recordDmsScheduledAutomationSuccess_('sendTelegramMorningDigest', event, 'sent');
  return 'Утренняя сводка отправлена.';
}

function sendTelegramDailyQueue(event) {
  assertDmsP1ReleaseReady_();
  assertDmsScheduledInvocation_('sendTelegramDailyQueue', event);
  const settings = getTelegramFinalSettings_();
  if (!settings.evening) {
    recordDmsScheduledAutomationSuccess_('sendTelegramDailyQueue', event, 'disabled');
  ...
  if (!dashboard.items.length) {
    recordDmsScheduledAutomationSuccess_('sendTelegramDailyQueue', event, 'empty');
  ...
  telegramSendMessage_(getTelegramProperty_(DMS_TELEGRAM.PROP_CHAT_ID),
    ...
    dashboard.replyMarkup);
  recordDmsScheduledAutomationSuccess_('sendTelegramDailyQueue', event, 'sent');
...
function createDmsAutomaticBackup(event) {
  assertDmsScheduledInvocation_('createDmsAutomaticBackup', event);
  const backupId = createTelegramDataBackup();
  console.log('Автоматическая резервная копия создана: ' + backupId);
  recordDmsScheduledAutomationSuccess_('createDmsAutomaticBackup', event, 'completed');
  return backupId;
}
