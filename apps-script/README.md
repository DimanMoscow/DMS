...
- `versions/v39` is the numbered dry-run-first candidate and is not deployed.
...
file. When the local exact exports are present, the verifier proves that every
repository file is byte-identical to its export after exactly the two documented
URL replacements above.
...
production deployment remains on `v38`; publishing `v39`, connecting clasp for
writes, or changing the deployment requires a separate approved operation.
