...
| Client portal bootstrap | `client_portal_bootstrap` | `handleDmsClientPortalRequest_` | Access, Clients, Blocks, Journal, Payments, Measurements | none | none | HTTP response | redacted API failure; candidate isolates malformed measurements to their owning client and reports aggregate corruption health |
| Client portal enrollment | `client_portal_enroll` | `handleDmsClientPortalEnrollmentRequest_` | Invitations, Access, client record | Access binding; invitation state/timestamps | shared mutation lock | binding activation | invitation and access sheet records; candidate resolves the unique active binding while retaining disabled history |
...
| Semantic financial reconciliation | self-tests and unified health | `computeDmsFinancialExpected_` plus `buildDmsSemanticReconciliationHealth_` | Clients, Blocks, Journal, Payments and financial formula anchors | none | none | none | redacted issue-class counts; candidate detects duplicate records, missing references, cross-client block ownership, numeric drift and formula drift |
...
5. Candidate health now preserves `healthy`, `not_due_yet`, `awaiting_sync`, `delayed`, `failed`, `stale` and `manual_review` component states and exposes runtime/version, scheduled handlers, reconciliation, pending operations, contention, error classes and Vercel connectivity without identifiers. Production remains on v54 until the milestone release.
6. Candidate semantic reconciliation now covers Client → Block → Journal → Payment ownership, missing references, duplicate accounting IDs, numeric results and formula anchors as read-only detection. It performs no automatic financial repair. Disabled-binding re-enrollment and measurement corruption isolation are also covered.
...
3. **Canonical queue decision** — implemented in the stabilization candidate with shared result, durable MiniApp replay and Telegram/MiniApp parity/fault tests.
