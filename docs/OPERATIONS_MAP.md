...
6. Add semantic reconciliation beyond count parity. The candidate now covers disabled-binding re-enrollment and isolates malformed measurement rows to the affected client while surfacing redacted corruption counts in health.
...
7. **P2 reliability** — binding lifecycle and measurement corruption isolation are implemented in the candidate; semantic reconciliation and the acceptance matrix remain.
