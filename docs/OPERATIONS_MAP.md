...
| Queue decision | Telegram `qd:*`; MiniApp `set_queue_decision`; move workflow | `setTelegramQueueDecision_` is shared only for the core decision | Queue | decision/status/source/comment | shared mutation lock; Telegram also has `cf2` lifecycle | Telegram message refresh or starts Calendar move | Telegram operation ledger and queue `[tgop:*]`; MiniApp has no durable operation ledger |
| Day confirmation | Telegram `qp:*`; MiniApp `confirm_day`; spreadsheet `processSelectedQueueDay` | `processQueueDate_` is the shared queue processor | Queue, Clients, Blocks, Journal; MiniApp also builds Calendar plan | Journal, Queue, Blocks, Clients; Calendar cancellations | shared mutation lock; Telegram also has `cf2` lifecycle | Calendar delete; Telegram edit | Telegram operation ledger/action audit; MiniApp returns result only; sheet path logs in rows |
...
| D2 | high | Day confirmation | Telegram and MiniApp share `processQueueDate_`, but independently orchestrate accepted input, Calendar sync boundary, Calendar cancellation, result shape and audit. MiniApp builds/applies a fresh Calendar plan under the confirmation lock; Telegram intentionally processes the immutable accepted queue without syncing. Both choices are defensible, but the domain contract is not explicit or shared. |
| D3 | high | Queue decision | MiniApp repeats ID/decision/date/status validation and source writes around `setTelegramQueueDecision_`; Telegram adds `[tgop:*]`, durable replay and UI side effects. MiniApp has lock/idempotent-value behavior only, so the same business command has different replay evidence and results. |
...
3. **Canonical queue decision** — immutable transport-neutral command/result and shared durable execution adapter; Telegram and MiniApp parity tests.
4. **Canonical day confirmation** — one orchestration contract with explicit accepted snapshot/sync policy, shared idempotency and fault-injection tests.
